<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title><?php echo $title; ?></title>
    <link href="https://fonts.googleapis.com/css?family=Montserrat:400,700|Raleway:500|Roboto|Roboto+Slab" rel="stylesheet" />
    <link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.0.13/css/all.css" integrity="sha384-DNOHZ68U8hZfKXOrtjWvjxusGo9WQnrNx2sqG0tfsghAvtVlRW3tvkXWZh58N9jp"
          crossorigin="anonymous" />
    <link rel="stylesheet" type="text/css" href="//cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick.css" />
    <link rel="stylesheet" type="text/css" href="//cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick-theme.css" />
    <link rel="stylesheet" href="<?php echo base_url(); ?>assets/main.css">
</head>

<body>
<nav>
    <div class="contact-links-wrapper">
        <ul class="contact-links">
            <li>
                <a href="">
                    <i class="fas fa-phone" style="transform: rotateY(180deg);transform-origin:  center center;"></i>
                    <span>(610)-183-4563</span>
                </a>
            </li>
            <li>
                <a href="">
                    <i class="far fa-envelope"></i>
                    <span>mail@motorius.com</span>
                </a>
            </li>
        </ul>
    </div>
    <div class="social-links-wrapper">
        <ul class="social-links">
            <li>
                <a href="">
                    <i class="fab fa-facebook-f"></i>
                </a>
            </li>
            <li>
                <a href="">
                    <i class="fab fa-twitter"></i>
                </a>
            </li>
            <li>
                <a href="">
                    <i class="fab fa-google-plus-g"></i>
                </a>
            </li>
            <li>
                <a href="">
                    <i class="fab fa-linkedin-in"></i>
                </a>
            </li>
        </ul>
    </div>

    <div class="site-nav__left">
        <div class="logo">
            <div class="gear"></div>
            Motor
            <span>ius</span>
        </div>

    </div>
    <div class="site-nav__right">
        <ul class="nav-list">
            <li>
                <a href="#">Home</a>
            </li>
            <li>
                <a href="#">Listing</a>
            </li>
            <li>
                <a href="#">Services</a>
            </li>
        </ul>
    </div>
</nav>
<header class="site-header">

    <div class="container site-header__center">
        <h4>The Power of Elegance</h4>
        <h1>Firrari California T 2015</h1>
        <h3>Starting at
            <span>$78</span>
        </h3>
        <button class="site-header__cta">Rent Now
            <img src="<?php echo base_url(); ?>assets/images/icons/right-arrow.svg" height="13px">
        </button>
    </div>
</header>
<main>
    <section class="main-intro">
        <!-- <div class="vehicle-search">
          <header class="vehicle-search__header">
            <div class="container">
              <p class="vehicle-search__title">Reserve you
                <span>Rental Car</span>
              </p>
              <p class="vehicle-search__stats">
                <span>9533</span> vehicles to rent</p>
            </div>
          </header>
          <div class="vehicle-search__body">
            <div class="container">
              <form class="vehicle-search__form">
                <div class="form-group col-5" data-colums="5">
                  <label class="">
                    <span>Pick-up Location</span>
                    <input type="text" name="pick-up-location">
                  </label>
                </div>
                <div class="form-group col-3" data-colums="3">
                  <label>
                    <span>Pick-up Date</span>
                    <input type="text" name="pick-up-date">
                  </label>
                  <label>
                    <span>Pick-up Time</span>
                    <input type="text" name="pick-up-time">
                  </label>
                </div>
                <div class="form-group col-5" data-colums="5">
                  <label class="form-input__pick-up-location">
                    <span>Return Location</span>
                    <input type="text" name="pick-up-location">
                  </label>
                </div>
                <div class="form-group col-3" data-colums="3">
                  <label>
                    <span>Return Date</span>
                    <input type="text" name="pick-up-location">
                  </label>
                  <label>
                    <span>Return Time</span>
                    <input type="text" name="pick-up-location">
                  </label>
                </div>
                <div class="form-group col-2 advanced-options" data-colums="2">
                  <label class="advanced-options">
                    <span>Class Car</span>
                    <input type="text" name="pick-up-location">
                  </label>
                </div>
                <div class="form-group col-2 advanced-options" data-colums="2">
                  <label>
                    <span>Body Style</span>
                    <input type="text" name="pick-up-location">
                  </label>
                </div>
                <div class="form-group col-2 advanced-options" data-colums="2">
                  <label>
                    <span>Make</span>
                    <input type="text" name="pick-up-location">
                  </label>
                </div>
                <div class="form-group col-2 advanced-options" data-colums="2">
                  <label>
                    <span>Year</span>
                    <input type="text" name="pick-up-location">
                  </label>
                </div>
              </form>
              <div class="vehicle-search__form-submit">
                <ul>
                  <li>No credit card fees</li>
                  <li>No ammendment fees</li>
                </ul>
                <button>select my car</button>
              </div>
            </div>
          </div>
        </div> -->
        <div class="vehicle-service">
            <div class="vehicle-service__analyse">
                <img src="<?php echo base_url(); ?>assets/images/icons/car-analyse.svg" alt="">
                <div class="vehicle-service__description">
                    <h3>Aliquam congue</h3>
                    <p>Vestibulum laoreet dolor justo, at semper dolor fringilla quis. Vivamus ut velit ac arcu imperdiet malesuada.
                    </p>
                </div>
            </div>
            <div class="vehicle-service__verification">
                <div class="vehicle-service__description">
                    <h3>Aliquam congue</h3>
                    <p>Vestibulum laoreet dolor justo, at semper dolor fringilla quis. Vivamus ut velit ac arcu imperdiet malesuada.
                    </p>
                </div>
                <img src="<?php echo base_url(); ?>assets/images/icons/car-verified.svg" alt="">
            </div>
        </div>
        <div class="about-us">
            <h2>About
                <span>Motorius</span>
            </h2>
            <p>Nulla tempor tempor egestas. Praesent pretium ultricies volutpat. Nulla eu nulla ac metus faucibus commodo. Aliquam
                congue eleifend lectus at congue. Aenean vel mi nisi. Vivamus vitae ante a tellus lobortis rhoncus non id dolor.
                In dictum leo quis nisi convallis, eleifend porttitor ex ornare.
            </p>
            <a href="#">Read More</a>
        </div>
        <div class="car-models">
            <div class="container">
                <!-- "car-models__slider" is important for the slider to work -->
                <ul class="car-models__slider">
                    <li>
                        <img src="<?php echo base_url(); ?>assets/images/cars-logo/toyota.png" alt="" width="150px">
                    </li>
                    <li>
                        <img src="<?php echo base_url(); ?>assets/images/cars-logo/peugeot.png" alt="" width="150px">
                    </li>
                    <li>
                        <img src="<?php echo base_url(); ?>assets/images/cars-logo/hyundai.png" alt="" width="150px">
                    </li>
                    <li>
                        <img src="<?php echo base_url(); ?>assets/images/cars-logo/mercedes-benz.png" alt="" width="150px">
                    </li>
                    <li>
                        <img src="<?php echo base_url(); ?>assets/images/cars-logo/bmw.png" alt="" width="150px">
                    </li>
                    <li>
                        <img src="<?php echo base_url(); ?>assets/images/cars-logo/toyota.png" alt="" width="150px">
                    </li>
                    <li>
                        <img src="<?php echo base_url(); ?>assets/images/cars-logo/peugeot.png" alt="" width="150px">
                    </li>
                    <li>
                        <img src="<?php echo base_url(); ?>assets/images/cars-logo/hyundai.png" alt="" width="150px">
                    </li>
                    <li>
                        <img src="<?php echo base_url(); ?>assets/images/cars-logo/mercedes-benz.png" alt="" width="150px">
                    </li>
                    <li>
                        <img src="<?php echo base_url(); ?>assets/images/cars-logo/bmw.png" alt="" width="150px">
                    </li>
                </ul>
            </div>
        </div>
    </section>
    <section class="newest-vehicles">
        <header class="newest-vehicles__header">
            <div class="container">
                <div class="newest-vehicles__header-left">
                    <h3>Newest
                        <span>Vehichles</span>
                    </h3>
                </div>
                <div class="newest-vehicles__header-right">
                    <a href="#">view all vehicles</a>
                </div>
            </div>
        </header>
        <div class="newest-vehicles__grid container">
            <!--
              data-area attribute is important, it used to position each
              vehicle in the grid. Acceptable values are:
                top-left, top-right, btm-left, btm-right
            -->
            <div class="new-vehicle" data-area="top-left">
                <div class="new-vehicle__image">
                    <img src="<?php echo base_url(); ?>assets/images/cars/01.png" alt="">
                </div>
                <div class="new-vehicle__info">
                    <p class="new-vehicle__name">2016 Marsedes-Benz AMG GLA</p>
                    <p class="new-vehicle__price">$55</p>
                </div>
            </div>
            <div class="new-vehicle" data-area="top-right">
                <div class="new-vehicle__image">
                    <img src="<?php echo base_url(); ?>assets/images/cars/02.png" alt="">
                </div>
                <div class="new-vehicle__info">
                    <p class="new-vehicle__name">2016 Marsedes-Benz AMG GLA</p>
                    <p class="new-vehicle__price">$55</p>
                </div>
            </div>
            <div class="new-vehicle" data-area="btm-left">
                <div class="new-vehicle__image">
                    <img src="<?php echo base_url(); ?>assets/images/cars/03.png" alt="">
                </div>
                <div class="new-vehicle__info">
                    <p class="new-vehicle__name">2016 Marsedes-Benz AMG GLA</p>
                    <p class="new-vehicle__price">$55</p>
                </div>
            </div>
            <div class="new-vehicle" data-area="btm-right">
                <div class="new-vehicle__image">
                    <img src="<?php echo base_url(); ?>assets/images/cars/04.png" alt="">
                </div>
                <div class="new-vehicle__info">
                    <p class="new-vehicle__name">2016 Marsedes-Benz AMG GLA</p>
                    <p class="new-vehicle__price">$55</p>
                </div>
            </div>
        </div>
        <p class="newest-vehicles__disclaimer container">All rates are per day, including taxes</p>
        <div class="newest-vehicles__outro">
            <div class="newest-vehicles__clipped">
                <ul>
                    <li data-bg-url="<?php echo base_url(); ?>assets/images/icons/gears.svg">
                        <img src="<?php echo base_url(); ?>assets/images/icons/gears.svg" width="50px" height="50px">
                        <div>
                            <h4>Maecenas id nibh viverra</h4>
                            <p>Maecenas sed quam tortor. Vivamus vitae cursus nisi, nec commodo orci. Duis sodales auctor nulla vel consectetur.
                            </p>
                        </div>
                    </li>
                    <li>
                        <img src="<?php echo base_url(); ?>assets/images/icons/tools.svg" width="50px" height="50px">
                        <div>
                            <h4>Integer placerat purus</h4>
                            <p>Phasellus sem est, ultrices rutrum nisi et, volutpat scelerisque metus. Nullam eu enim et eros efficitur
                                fringilla.
                            </p>
                        </div>
                    </li>
                    <li>
                        <img src="<?php echo base_url(); ?>assets/images/icons/perf.svg" width="55px" height="55px">
                        <div>
                            <h4>Etiam ac ante iaculis</h4>
                            <p>Integer egestas lorem quis risus blandit, nec ultrices nulla tincidunt. Sed venenatis odio ut est semper
                                sollicitudin.
                            </p>
                        </div>
                    </li>
                </ul>
            </div>
        </div>
    </section>
    <section class="all-vehicles">
        <header>
            <div class="all-vehicles__intro container">
                <h3>All
                    <span>Vehichles</span>
                </h3>
                <p>Nulla mollis urna enim, ac maximus leo blandit in. Maecenas tincidunt nibh odio, non posuere erat suscipit ac.
                    Etiam libero turpis, vestibulum eget nibh iaculis, convallis aliquet quam.
                </p>
            </div>
            <!-- <div class="all-vehicles__filter">
              <div class="container">
                <p>Filter By</p>
                <ul>
                  <li>
                    <button>Body style</button>
                  </li>
                  <li>
                    <button>Class</button>
                  </li>
                  <li>
                    <button>Make</button>
                  </li>
                </ul>
              </div>
            </div> -->
        </header>
        <div class="all-vehicles__grid container">
            <div class="all-vehicles__grid-item vehicle">
                <div class="vehicle__image">
                    <img src="<?php echo base_url(); ?>assets/images/cars/05.png" alt="">
                </div>
                <div class="vehicle__info">
                    <p class="vehicle__name">2015 Ferrari California</p>
                    <div class="vehicle__rating">
                        <i class="fas fa-star checked"></i>
                        <i class="fas fa-star checked"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                    </div>
                    <p class="vehicle__pricing">Starting at
                        <span class="price">$55</span> / day</p>
                </div>
            </div>
            <div class="all-vehicles__grid-item vehicle">
                <div class="vehicle__image">
                    <img src="<?php echo base_url(); ?>assets/images/cars/06.png" alt="">
                </div>
                <div class="vehicle__info">
                    <p class="vehicle__name">2015 Ferrari California</p>
                    <div class="vehicle__rating">
                        <i class="fas fa-star checked"></i>
                        <i class="fas fa-star checked"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                    </div>
                    <p class="vehicle__pricing">Starting at
                        <span class="price">$55</span> / day</p>
                </div>
            </div>
            <div class="all-vehicles__grid-item vehicle">
                <div class="vehicle__image">
                    <img src="<?php echo base_url(); ?>assets/images/cars/07.png" alt="">
                </div>
                <div class="vehicle__info">
                    <p class="vehicle__name">2015 Ferrari California</p>
                    <div class="vehicle__rating">
                        <i class="fas fa-star checked"></i>
                        <i class="fas fa-star checked"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                    </div>
                    <p class="vehicle__pricing">Starting at
                        <span class="price">$55</span> / day</p>
                </div>
            </div>
            <div class="all-vehicles__grid-item vehicle">
                <div class="vehicle__image">
                    <img src="<?php echo base_url(); ?>assets/images/cars/08.png" alt="">
                </div>
                <div class="vehicle__info">
                    <p class="vehicle__name">2015 Ferrari California</p>
                    <div class="vehicle__rating">
                        <i class="fas fa-star checked"></i>
                        <i class="fas fa-star checked"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                    </div>
                    <p class="vehicle__pricing">Starting at
                        <span class="price">$55</span> / day</p>
                </div>
            </div>
            <div class="all-vehicles__grid-item vehicle">
                <div class="vehicle__image">
                    <img src="<?php echo base_url(); ?>assets/images/cars/09.png" alt="">
                </div>
                <div class="vehicle__info">
                    <p class="vehicle__name">2015 Ferrari California</p>
                    <div class="vehicle__rating">
                        <i class="fas fa-star checked"></i>
                        <i class="fas fa-star checked"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                    </div>
                    <p class="vehicle__pricing">Starting at
                        <span class="price">$55</span> / day</p>
                </div>
            </div>
            <div class="all-vehicles__grid-item vehicle">
                <div class="vehicle__image">
                    <img src="<?php echo base_url(); ?>assets/images/cars/10.png" alt="">
                </div>
                <div class="vehicle__info">
                    <p class="vehicle__name">2015 Ferrari California</p>
                    <div class="vehicle__rating">
                        <i class="fas fa-star checked"></i>
                        <i class="fas fa-star checked"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                    </div>
                    <p class="vehicle__pricing">Starting at
                        <span class="price">$55</span> / day</p>
                </div>
            </div>
            <div class="all-vehicles__grid-item vehicle">
                <div class="vehicle__image">
                    <img src="<?php echo base_url(); ?>assets/images/cars/11.png" alt="">
                </div>
                <div class="vehicle__info">
                    <p class="vehicle__name">2015 Ferrari California</p>
                    <div class="vehicle__rating">
                        <i class="fas fa-star checked"></i>
                        <i class="fas fa-star checked"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                    </div>
                    <p class="vehicle__pricing">Starting at
                        <span class="price">$55</span> / day</p>
                </div>
            </div>
            <div class="all-vehicles__grid-item vehicle">
                <div class="vehicle__image">
                    <img src="<?php echo base_url(); ?>assets/images/cars/12.png" alt="">
                </div>
                <div class="vehicle__info">
                    <p class="vehicle__name">2015 Ferrari California</p>
                    <div class="vehicle__rating">
                        <i class="fas fa-star checked"></i>
                        <i class="fas fa-star checked"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                    </div>
                    <p class="vehicle__pricing">Starting at
                        <span class="price">$55</span> / day</p>
                </div>
            </div>
            <div class="all-vehicles__grid-item vehicle">
                <div class="vehicle__image">
                    <img src="<?php echo base_url(); ?>assets/images/cars/13.png" alt="">
                </div>
                <div class="vehicle__info">
                    <p class="vehicle__name">2015 Ferrari California</p>
                    <div class="vehicle__rating">
                        <i class="fas fa-star checked"></i>
                        <i class="fas fa-star checked"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                    </div>
                    <p class="vehicle__pricing">Starting at
                        <span class="price">$55</span> / day</p>
                </div>
            </div>
            <div class="all-vehicles__grid-item vehicle">
                <div class="vehicle__image">
                    <img src="<?php echo base_url(); ?>assets/images/cars/14.png" alt="">
                </div>
                <div class="vehicle__info">
                    <p class="vehicle__name">2015 Ferrari California</p>
                    <div class="vehicle__rating">
                        <i class="fas fa-star checked"></i>
                        <i class="fas fa-star checked"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                    </div>
                    <p class="vehicle__pricing">Starting at
                        <span class="price">$55</span> / day</p>
                </div>
            </div>
            <div class="all-vehicles__grid-item vehicle">
                <div class="vehicle__image">
                    <img src="<?php echo base_url(); ?>assets/images/cars/15.png" alt="">
                </div>
                <div class="vehicle__info">
                    <p class="vehicle__name">2015 Ferrari California</p>
                    <div class="vehicle__rating">
                        <i class="fas fa-star checked"></i>
                        <i class="fas fa-star checked"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                    </div>
                    <p class="vehicle__pricing">Starting at
                        <span class="price">$55</span> / day</p>
                </div>
            </div>
            <div class="all-vehicles__grid-item vehicle">
                <div class="vehicle__image">
                    <img src="<?php echo base_url(); ?>assets/images/cars/16.png" alt="">
                </div>
                <div class="vehicle__info">
                    <p class="vehicle__name">2015 Ferrari California</p>
                    <div class="vehicle__rating">
                        <i class="fas fa-star checked"></i>
                        <i class="fas fa-star checked"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                        <i class="fas fa-star"></i>
                    </div>
                    <p class="vehicle__pricing">Starting at
                        <span class="price">$55</span> / day</p>
                </div>
            </div>
        </div>
    </section>
    <section class="choosing-a-car">
        <div class="choosing-a-car__cta">
            <img src="<?php echo base_url(); ?>assets/images/icons/call-center.svg" width="65px" height="80px">
            <p>Do you
                <span>need help</span> in choosing a car?</p>
            <p>call us:
                <span>1-(672)-480-1829</span>
            </p>
        </div>
        <div class="choosing-a-car__car-types">
            <ul class="car-list">
                <li class="car-list__item">
                    <a href="">
                        <div class="car-list__item-image">
                            <svg xmlns="http://www.w3.org/2000/svg" width="98.594" height="28.94" viewBox="0 0 98.594 28.94">
                                <defs>
                                    <style>
                                        .cls-1 {
                                            fill: currentColor;
                                            fill-rule: evenodd
                                        }
                                    </style>
                                </defs>
                                <path id="Convertible" class="cls-1" d="M236.293 5563.63a7.234 7.234 0 1 0-14.466 0 7.38 7.38 0 0 0 .064.93 7.215 7.215 0 0 0 14.37-.3c.017-.21.032-.41.032-.63zm-10.856-2.77l1.476 1.5a2.528 2.528 0 0 0-.261.65h-2.08a4.662 4.662 0 0 1 .865-2.15zm-.871 3.42h2.094a2.36 2.36 0 0 0 .262.64l-1.477 1.5a4.579 4.579 0 0 1-.879-2.14zm3.869 3.92a4.462 4.462 0 0 1-2.1-.88l1.47-1.5a2.32 2.32 0 0 0 .629.26v2.12zm0-7.01a2.436 2.436 0 0 0-.641.27l-1.472-1.49a4.413 4.413 0 0 1 2.113-.89v2.11zm1.25-2.12a4.625 4.625 0 0 1 2.114.89l-1.472 1.5a2.414 2.414 0 0 0-.642-.27v-2.12zm0 9.13v-2.12a2.653 2.653 0 0 0 .629-.26l1.472 1.5a4.473 4.473 0 0 1-2.101.88zm3-1.77l-1.483-1.51a2.592 2.592 0 0 0 .27-.64h2.078a4.619 4.619 0 0 1-.868 2.15zm-1.213-3.42a2.68 2.68 0 0 0-.261-.65l1.475-1.5a4.632 4.632 0 0 1 .866 2.15h-2.08zm56.436-6.73a7.291 7.291 0 0 0-7.232 7.35 7.382 7.382 0 0 0 .063.93 7.215 7.215 0 0 0 14.37-.3c.016-.2.031-.41.031-.63a7.289 7.289 0 0 0-7.24-7.35zm-3.622 4.58l1.476 1.5a2.567 2.567 0 0 0-.262.65h-2.079a4.641 4.641 0 0 1 .862-2.15zm-.872 3.42h2.094a2.392 2.392 0 0 0 .262.64l-1.478 1.5a4.588 4.588 0 0 1-.881-2.14zm3.869 3.92a4.466 4.466 0 0 1-2.1-.88l1.471-1.5a2.321 2.321 0 0 0 .627.26v2.12zm0-7.01a2.429 2.429 0 0 0-.64.27l-1.472-1.49a4.409 4.409 0 0 1 2.112-.89v2.11zm1.25-2.12a4.629 4.629 0 0 1 2.115.89l-1.472 1.5a2.434 2.434 0 0 0-.643-.27v-2.12zm0 9.13v-2.12a2.7 2.7 0 0 0 .63-.26l1.472 1.5a4.476 4.476 0 0 1-2.105.88zm3-1.77l-1.484-1.51a2.554 2.554 0 0 0 .27-.64h2.078a4.629 4.629 0 0 1-.869 2.15zm-1.214-3.42a2.68 2.68 0 0 0-.261-.65l1.475-1.5a4.641 4.641 0 0 1 .865 2.15h-2.079zm17.938-8.1a31.973 31.973 0 0 0-18.113-7.61l-5.694-1.82a1.534 1.534 0 0 0-1.926 1.02l-.453 1.44-10.474.8-.215-2.13a1.258 1.258 0 0 0-1.281-1.14l-.694.02a1.263 1.263 0 0 0-1.22 1.27v2.4l-15.725.73a1.976 1.976 0 0 0-.41-1.92l6.516-3.13a1.187 1.187 0 0 0 .045-2.11l-.763-.41a2.031 2.031 0 0 0-1.67-.1c-2.4.94-9.469 3.76-16.688 7.28-10.665.76-22.516 2.86-25.367 4.73s-3.126 4.18-2.915 6.57a7.6 7.6 0 0 0 6.277 6.86l4.671.77h.142a8.374 8.374 0 0 1-1.424-3.74 8.707 8.707 0 0 1-.071-1.06 8.254 8.254 0 1 1 16.507 0c0 .24-.016.48-.033.71a8.386 8.386 0 0 1-1.385 3.95l44.259-.45.607-.03a8.425 8.425 0 0 1-1.039-3.12 8.707 8.707 0 0 1-.071-1.06 8.254 8.254 0 1 1 16.507 0c0 .24-.015.48-.033.71a8.391 8.391 0 0 1-.783 2.89l10.181-.41.681-.76a14.859 14.859 0 0 0 3.465-6.86 4.476 4.476 0 0 0-1.416-4.29z"
                                      transform="translate(-211.156 -5542.06)" />
                            </svg>
                        </div>
                        <span>125</span>
                        <span>Convertible</span>
                    </a>
                </li>
                <li class="car-list__item">
                    <a href="">
                        <div class="car-list__item-image">
                            <svg xmlns="http://www.w3.org/2000/svg" width="98.781" height="34.47" viewBox="0 0 98.781 34.47">
                                <defs>
                                    <style>
                                        .cls-1 {
                                            fill: currentColor;
                                            fill-rule: evenodd
                                        }
                                    </style>
                                </defs>
                                <path id="Forma_1" data-name="Forma 1" class="cls-1" d="M460.337 5558.93a7.045 7.045 0 0 0-7.068 7.03 6.755 6.755 0 0 0 .063.89 7.066 7.066 0 0 0 14.044-.29c.016-.19.03-.39.03-.6a7.044 7.044 0 0 0-7.069-7.03zm-3.541 4.38l1.443 1.44a2.384 2.384 0 0 0-.256.62h-2.032a4.387 4.387 0 0 1 .849-2.06zm-.851 3.28h2.046a2.32 2.32 0 0 0 .256.6l-1.445 1.44a4.33 4.33 0 0 1-.857-2.04zm3.781 3.75a4.483 4.483 0 0 1-2.051-.85l1.437-1.43a2.187 2.187 0 0 0 .613.24v2.04zm0-6.72a2.658 2.658 0 0 0-.625.26l-1.439-1.43a4.312 4.312 0 0 1 2.064-.85v2.02zm1.221-2.02a4.311 4.311 0 0 1 2.067.85l-1.439 1.43a2.681 2.681 0 0 0-.628-.26v-2.02zm0 8.73v-2.03a2.175 2.175 0 0 0 .615-.24l1.438 1.43a4.355 4.355 0 0 1-2.053.84zm2.931-1.69l-1.45-1.45a2.338 2.338 0 0 0 .263-.6h2.031a4.4 4.4 0 0 1-.844 2.05zm-1.186-3.27a2.382 2.382 0 0 0-.255-.62l1.442-1.44a4.378 4.378 0 0 1 .846 2.06h-2.033zm58.706-6.89a7.051 7.051 0 0 0-7.066 7.04 6.557 6.557 0 0 0 .063.88 7.066 7.066 0 0 0 14.044-.28c.017-.2.03-.4.03-.6a7.055 7.055 0 0 0-7.069-7.04zm-3.541 4.38l1.443 1.44a2.348 2.348 0 0 0-.255.62h-2.033a4.387 4.387 0 0 1 .845-2.06zm-.851 3.28h2.047a2.543 2.543 0 0 0 .255.61l-1.444 1.44a4.4 4.4 0 0 1-.858-2.05zm3.782 3.75a4.482 4.482 0 0 1-2.052-.85l1.437-1.43a2.57 2.57 0 0 0 .614.25v2.03zm0-6.72a2.666 2.666 0 0 0-.626.26l-1.439-1.43a4.412 4.412 0 0 1 2.065-.85v2.02zm1.224-2.02a4.312 4.312 0 0 1 2.064.85l-1.439 1.43a2.676 2.676 0 0 0-.625-.26v-2.02zm0 8.74v-2.03a2.538 2.538 0 0 0 .612-.25l1.438 1.43a4.484 4.484 0 0 1-2.05.85zm2.928-1.7l-1.451-1.44a2.616 2.616 0 0 0 .265-.61h2.03a4.423 4.423 0 0 1-.844 2.05zm-1.185-3.27a2.422 2.422 0 0 0-.257-.62l1.443-1.44a4.4 4.4 0 0 1 .846 2.06h-2.032zm16.874-5.45l-.475-.21v-5.32a6.731 6.731 0 0 0-1.171-3.79l-4.124-6.04a9.094 9.094 0 0 0-6.284-3.87c-8.457-1.15-28.3-3.25-41.452.07-9.721 2.45-21.4 9.84-21.4 9.84s-15 2.43-18.309 4.24c-2.537 1.37-4.035 3.14-4.291 5.68a15.829 15.829 0 0 0 .36 4.17 3.921 3.921 0 0 0 3.114 3.1l5.519 1.06a8.66 8.66 0 0 1-.292-1.36 8.4 8.4 0 0 1-.074-1.08 8.591 8.591 0 0 1 17.181 0c0 .25-.016.49-.034.72a8.476 8.476 0 0 1-.357 1.79h44.81a8.351 8.351 0 0 1-.469-1.88 8.465 8.465 0 0 1-.074-1.08 8.591 8.591 0 0 1 17.181 0c0 .25-.016.49-.034.73a8.4 8.4 0 0 1-.509 2.23h.055l10.4-3.17a2.518 2.518 0 0 0 1.76-2.06l.211-1.55a2.111 2.111 0 0 0-1.242-2.22zm-29.672-18.72v8.53l-12.39.62v-9.29c.067 0 .131-.01.2-.01 4.251-.04 8.397.03 12.19.15zm-34.867 6.84v-.97a47.743 47.743 0 0 1 18.844-5.81l-1.082 9.32-17.762.89s.845-.39.845-2.27a1.445 1.445 0 0 0-.845-1.16zm37.231 1.58v-4.46l2.314 4.34zm17.485-1.39a5.733 5.733 0 0 1-2.33.63l-10.185.51-4.291-8.05c7.856.33 13.679.88 14.829 1.21 1.819.51 3.467 2.99 3.4 4.22a2.3 2.3 0 0 1-1.423 1.48z"
                                      transform="translate(-443.094 -5538.53)" />
                            </svg>
                        </div>
                        <span>125</span>
                        <span>Wagon</span>
                    </a>
                </li>
                <li class="car-list__item">
                    <a href="">
                        <div class="car-list__item-image">
                            <svg xmlns="http://www.w3.org/2000/svg" width="99.594" height="29.78" viewBox="0 0 99.594 29.78">
                                <defs>
                                    <style>
                                        .cls-1 {
                                            fill: #8d99ae;
                                            fill-rule: evenodd
                                        }
                                    </style>
                                </defs>
                                <path id="Forma_1" data-name="Forma 1" class="cls-1" d="M695.424 5558.11a7.457 7.457 0 0 0-7.476 7.44 7.3 7.3 0 0 0 .067.94 7.472 7.472 0 0 0 14.853-.3c.018-.21.032-.42.032-.64a7.456 7.456 0 0 0-7.476-7.44zm-3.745 4.64l1.525 1.52a2.517 2.517 0 0 0-.271.65h-2.15a4.639 4.639 0 0 1 .896-2.17zm-.9 3.46h2.165a2.586 2.586 0 0 0 .27.65l-1.528 1.52a4.644 4.644 0 0 1-.909-2.17zm4 3.97a4.62 4.62 0 0 1-2.168-.9l1.52-1.51a2.443 2.443 0 0 0 .648.26v2.15zm0-7.1a2.567 2.567 0 0 0-.661.27l-1.523-1.52a4.775 4.775 0 0 1 2.184-.9v2.15zm1.294-2.15a4.779 4.779 0 0 1 2.185.9l-1.524 1.52a2.539 2.539 0 0 0-.661-.27v-2.15zm0 9.25v-2.15a2.416 2.416 0 0 0 .648-.26l1.522 1.51a4.623 4.623 0 0 1-2.172.9zm3.1-1.8l-1.533-1.52a2.609 2.609 0 0 0 .279-.65h2.148a4.627 4.627 0 0 1-.899 2.17zm-1.253-3.46a2.485 2.485 0 0 0-.272-.65l1.525-1.52a4.609 4.609 0 0 1 .9 2.17h-2.149zm59.245-6.75a7.415 7.415 0 1 0 7.411 8.05c.019-.21.033-.42.033-.63a7.43 7.43 0 0 0-7.449-7.42zm-3.728 4.62l1.519 1.52a2.48 2.48 0 0 0-.27.65h-2.14a4.623 4.623 0 0 1 .886-2.17zm-.9 3.45h2.154a2.375 2.375 0 0 0 .27.64l-1.522 1.52a4.686 4.686 0 0 1-.905-2.16zm3.983 3.95a4.677 4.677 0 0 1-2.159-.89l1.513-1.51a2.43 2.43 0 0 0 .646.26v2.14zm0-7.07a2.5 2.5 0 0 0-.658.27l-1.517-1.51a4.628 4.628 0 0 1 2.175-.89v2.13zm1.289-2.13a4.615 4.615 0 0 1 2.174.89l-1.516 1.51a2.533 2.533 0 0 0-.658-.27v-2.13zm0 9.2v-2.14a2.41 2.41 0 0 0 .645-.26l1.514 1.51a4.677 4.677 0 0 1-2.162.89zm3.083-1.79l-1.527-1.52a2.536 2.536 0 0 0 .277-.64h2.139a4.584 4.584 0 0 1-.892 2.16zm-1.248-3.44a2.738 2.738 0 0 0-.27-.66l1.519-1.51a4.612 4.612 0 0 1 .891 2.17h-2.14zm18.3-4.55a5.143 5.143 0 0 0-.883-2.75l-.4-.6-1.76-4.62a2.415 2.415 0 0 0-2.075-1.56c-1.73-.15-4.672-.37-7.48-.47-9.814-5.2-27.961-10.41-43.35-4.73-3.158 1.17-12.18 5.74-12.18 5.74s-14.65-.37-25.1 3.34a9.474 9.474 0 0 0-6.278 9.86 22.5 22.5 0 0 0 .342 2.26 2.309 2.309 0 0 0 2.018 1.84l6.5.71a8.879 8.879 0 0 1-.821-2.75 8.708 8.708 0 0 1-.078-1.13 9.028 9.028 0 0 1 18.055 0c0 .26-.017.52-.038.77a8.838 8.838 0 0 1-.871 3.13h45.138l.367-.02a8.813 8.813 0 0 1-.806-2.72 8.669 8.669 0 0 1-.077-1.12 9 9 0 0 1 17.991 0c0 .26-.017.51-.038.76a8.861 8.861 0 0 1-.454 2.11l1.017-.06 6.993-.93a3.944 3.944 0 0 0 2.9-1.93l.7-1.19a5.188 5.188 0 0 0 .7-2.73zm-24.772-8.91H718.94a2.075 2.075 0 0 0-.692-1.78s8.751-5.63 22.265-4.22c3.719.38 8.263.65 14.45 3.85z"
                                      transform="translate(-678.375 -5543.22)" />
                            </svg>
                        </div>
                        <span>125</span>
                        <span>Coupe</span>
                    </a>
                </li>
                <li class="car-list__item">
                    <a href="">
                        <div class="car-list__item-image">
                            <svg xmlns="http://www.w3.org/2000/svg" width="98.94" height="46.97" viewBox="0 0 98.94 46.97">
                                <defs>
                                    <style>
                                        .cls-1 {
                                            fill: #8d99ae;
                                            fill-rule: evenodd
                                        }
                                    </style>
                                </defs>
                                <path id="Forma_1" data-name="Forma 1" class="cls-1" d="M919.052 5551.75a10.58 10.58 0 0 0-10.545 10.62 10.334 10.334 0 0 0 .094 1.34 10.528 10.528 0 0 0 20.95-.43c.024-.3.045-.6.045-.91a10.579 10.579 0 0 0-10.544-10.62zm-3.965 7.61l1.615 1.63a2.716 2.716 0 0 0-.287.7h-2.276a5.016 5.016 0 0 1 .948-2.33zm-.958 3.72h2.294a2.839 2.839 0 0 0 .287.69l-1.619 1.63a5.062 5.062 0 0 1-.962-2.32zm4.238 4.25a4.831 4.831 0 0 1-2.3-.97l1.608-1.62a2.43 2.43 0 0 0 .688.28v2.31zm0-7.61a2.5 2.5 0 0 0-.7.29l-1.612-1.62a4.881 4.881 0 0 1 2.312-.97v2.3zm1.372-2.3a4.872 4.872 0 0 1 2.312.97l-1.613 1.62a2.484 2.484 0 0 0-.7-.29v-2.3zm0 9.9v-2.3a2.412 2.412 0 0 0 .687-.28l1.609 1.62a4.818 4.818 0 0 1-2.296.96zm3.278-1.92l-1.624-1.63a2.865 2.865 0 0 0 .295-.69h2.276a4.987 4.987 0 0 1-.947 2.32zm-1.326-3.71a2.788 2.788 0 0 0-.287-.7l1.616-1.63a5.046 5.046 0 0 1 .947 2.33h-2.276zm62.465-9.94a10.581 10.581 0 0 0-10.545 10.62 10.334 10.334 0 0 0 .094 1.34 10.529 10.529 0 0 0 20.951-.43c.023-.3.045-.6.045-.91a10.58 10.58 0 0 0-10.545-10.62zm-3.965 7.61l1.615 1.63a2.751 2.751 0 0 0-.287.7h-2.275a5 5 0 0 1 .947-2.33zm-.957 3.72h2.294a2.836 2.836 0 0 0 .286.69l-1.619 1.63a5.05 5.05 0 0 1-.961-2.32zm4.237 4.25a4.823 4.823 0 0 1-2.3-.97l1.609-1.62a2.4 2.4 0 0 0 .687.28v2.31zm0-7.61a2.5 2.5 0 0 0-.7.29l-1.612-1.62a4.872 4.872 0 0 1 2.312-.97v2.3zm1.373-2.3a4.873 4.873 0 0 1 2.311.97l-1.612 1.62a2.521 2.521 0 0 0-.7-.29v-2.3zm0 9.9v-2.3a2.388 2.388 0 0 0 .685-.28l1.611 1.62a4.822 4.822 0 0 1-2.296.96zm3.277-1.92l-1.624-1.63a2.865 2.865 0 0 0 .3-.69h2.276a4.966 4.966 0 0 1-.952 2.32zm-1.326-3.71a2.751 2.751 0 0 0-.287-.7l1.616-1.63a5.036 5.036 0 0 1 .947 2.33H986.8zm-7-35.65l-28.29.36a1.545 1.545 0 0 0 0 3.09h.038l28.29-.36a1.545 1.545 0 0 0-.042-3.09zm-11.195 36.04h-33.4a1.55 1.55 0 0 0 0 3.1h33.4a1.55 1.55 0 0 0 0-3.1zm33.9-8.83a1.874 1.874 0 0 0-1.46-.69h-1.74l-1.774-19.57a2.057 2.057 0 0 0-2.082-1.88l-49.948.75a2.55 2.55 0 0 0-1.985 1l-6.621 8.57-21.679 1.35a3.786 3.786 0 0 0-2.921 1.68l-1.221 1.81a14.553 14.553 0 0 0-2.651 1.2 12.715 12.715 0 0 0-1.453 2.08.863.863 0 0 0-.059.89.844.844 0 0 0 .755.47v1.9h-1.8a1.864 1.864 0 0 0-1.855 1.87v3.44a2.311 2.311 0 0 0 2.3 2.32h1.233l3.178-8.83a2.024 2.024 0 0 1 1.9-1.34H923.7a2.023 2.023 0 0 1 1.6.8l7.472 9.88h39l5.473-9.3a2.01 2.01 0 0 1 1.735-1h10.186a2.03 2.03 0 0 1 1.744 1.01l5.366 9.29h1.925a4.249 4.249 0 0 0 4.162-3.44l.52-2.68a1.911 1.911 0 0 0-.383-1.58zm-47.515-9.85h-11.51v-3.56h-2.331l3.488-4.64a.791.791 0 0 1 .632-.32l11.675-.17v5.13zm19.032 0h-12.412v-8.76l14.969-.23v6.42zm6.4 0v-9.05l12.886-.19a.419.419 0 0 1 .418.39l.605 8.85h-13.912z"
                                      transform="translate(-904 -5526.03)" />
                            </svg>
                        </div>
                        <span>125</span>
                        <span>suv</span>
                    </a>
                </li>
                <li class="car-list__item">
                    <a href="">
                        <div class="car-list__item-image">
                            <svg xmlns="http://www.w3.org/2000/svg" width="99.85" height="39.06" viewBox="0 0 99.85 39.06">
                                <defs>
                                    <style>
                                        .cls-1 {
                                            fill: #8d99ae;
                                            fill-rule: evenodd
                                        }
                                    </style>
                                </defs>
                                <path id="Forma_1" data-name="Forma 1" class="cls-1" d="M1158.28 5557.83a7.577 7.577 0 0 0-7.59 7.58 7.341 7.341 0 0 0 .06.95 7.583 7.583 0 0 0 15.08-.3c.02-.22.04-.43.04-.65a7.584 7.584 0 0 0-7.59-7.58zm-3.81 4.72l1.55 1.55a2.614 2.614 0 0 0-.27.67h-2.18a4.8 4.8 0 0 1 .9-2.22zm-.91 3.53h2.2a2.546 2.546 0 0 0 .27.66l-1.55 1.55a4.684 4.684 0 0 1-.92-2.21zm4.06 4.04a4.635 4.635 0 0 1-2.2-.91l1.54-1.54a2.523 2.523 0 0 0 .66.26v2.19zm0-7.23a2.322 2.322 0 0 0-.67.28l-1.55-1.55a4.707 4.707 0 0 1 2.22-.91v2.18zm1.31-2.18a4.666 4.666 0 0 1 2.22.91l-1.54 1.55a2.382 2.382 0 0 0-.68-.28v-2.18zm0 9.41v-2.19a2.392 2.392 0 0 0 .66-.26l1.55 1.54a4.671 4.671 0 0 1-2.21.91zm3.15-1.83l-1.56-1.55a3.005 3.005 0 0 0 .29-.66h2.18a4.774 4.774 0 0 1-.91 2.21zm-1.27-3.52a2.639 2.639 0 0 0-.28-.67l1.55-1.55a4.707 4.707 0 0 1 .91 2.22h-2.18zm62.59-6.94a7.584 7.584 0 0 0-7.59 7.58 7.319 7.319 0 0 0 .07.95 7.583 7.583 0 0 0 15.08-.3c.02-.22.03-.43.03-.65a7.584 7.584 0 0 0-7.59-7.58zm-3.8 4.72l1.55 1.55a2.639 2.639 0 0 0-.28.67h-2.18a4.707 4.707 0 0 1 .91-2.22zm-.92 3.53h2.2a2.546 2.546 0 0 0 .27.66l-1.55 1.55a4.788 4.788 0 0 1-.92-2.21zm4.06 4.04a4.635 4.635 0 0 1-2.2-.91l1.54-1.54a2.668 2.668 0 0 0 .66.26v2.19zm0-7.23a2.322 2.322 0 0 0-.67.28l-1.55-1.55a4.707 4.707 0 0 1 2.22-.91v2.18zm1.32-2.18a4.707 4.707 0 0 1 2.22.91l-1.55 1.55a2.322 2.322 0 0 0-.67-.28v-2.18zm0 9.41v-2.19a2.668 2.668 0 0 0 .66-.26l1.54 1.54a4.635 4.635 0 0 1-2.2.91zm3.14-1.83l-1.55-1.55a2.572 2.572 0 0 0 .28-.66h2.18a4.774 4.774 0 0 1-.91 2.21zm-1.27-3.52a2.614 2.614 0 0 0-.27-.67l1.54-1.55a4.707 4.707 0 0 1 .91 2.22h-2.18zm12.66-13.09a10.345 10.345 0 0 0-1.35-4.77l-5.45-9.61a5.3 5.3 0 0 0-4.34-2.68c-9.13-.45-37.15-1.62-45.97.75-5.96 1.6-21.01 11.49-21.01 11.49s-16.57 2.57-19.73 10.11a3.863 3.863 0 0 0-1.61 2.84 12.89 12.89 0 0 0 .53 3.98 3.8 3.8 0 0 0 3.08 2.79l7.19 1.12a7.984 7.984 0 0 1-.25-1.2 7.766 7.766 0 0 1-.08-1.09 8.675 8.675 0 0 1 17.35 0c0 .25-.02.49-.04.74a8.16 8.16 0 0 1-.57 2.41l.85.05 47.76-1.29c-.06-.27-.11-.54-.15-.82a9.659 9.659 0 0 1-.07-1.09 8.67 8.67 0 0 1 17.34 0c0 .25-.01.49-.03.74-.02.16-.05.31-.07.47l4.13-.84a3.553 3.553 0 0 0 2.84-3.6zm-27.49-15.28l1.1 10.94-16.84.69v-11.69c4.92-.11 10.54-.06 15.74.06zm-42.49 12.41l-1.67.1a1.028 1.028 0 0 1-1.02-.68 1.039 1.039 0 0 1 .36-1.17c.98-.71 2.18-1.56 3.51-2.47v2.8a5 5 0 0 0-1.18 1.42zm4.12-2.56c-.1.01-.19.03-.29.04v-3.43a50.975 50.975 0 0 1 10.55-5.36 36.606 36.606 0 0 1 7.67-.98v11.7l-14.47.59a3.261 3.261 0 0 0-3.46-2.56zm58.5-1.06a2.93 2.93 0 0 1-2.43 1.48l-11.9.48-1.07-10.62c4.92.15 9.03.33 11.11.44a2.753 2.753 0 0 1 2.28 1.46l2.07 3.92a2.928 2.928 0 0 1-.06 2.84z"
                                      transform="translate(-1139.09 -5533.94)" />
                            </svg>
                        </div>
                        <span>125</span>
                        <span>Minivans</span>
                    </a>
                </li>
                <li class="car-list__item">
                    <a href="">
                        <div class="car-list__item-image">
                            <svg xmlns="http://www.w3.org/2000/svg" width="99.75" height="33" viewBox="0 0 99.75 33">
                                <defs>
                                    <style>
                                        .cls-1 {
                                            fill: #8d99ae;
                                            fill-rule: evenodd
                                        }
                                    </style>
                                </defs>
                                <path id="Forma_1" data-name="Forma 1" class="cls-1" d="M1390.13 5555.26a7.87 7.87 0 1 0 7.91 8.54c.02-.22.03-.44.03-.67a7.9 7.9 0 0 0-7.94-7.87zm-3.97 4.91l1.62 1.6a2.6 2.6 0 0 0-.29.69h-2.28a4.637 4.637 0 0 1 .95-2.29zm-.96 3.66h2.3a2.707 2.707 0 0 0 .28.68l-1.62 1.61a4.927 4.927 0 0 1-.96-2.29zm4.24 4.19a4.937 4.937 0 0 1-2.3-.94l1.62-1.6a2.682 2.682 0 0 0 .68.27v2.27zm0-7.51a3.564 3.564 0 0 0-.7.29l-1.61-1.6a4.938 4.938 0 0 1 2.32-.95v2.26h-.01zm1.38-2.26a4.938 4.938 0 0 1 2.32.95l-1.62 1.6a3.122 3.122 0 0 0-.7-.29v-2.26zm0 9.77v-2.27a2.442 2.442 0 0 0 .69-.28l1.61 1.61a5 5 0 0 1-2.3.94zm3.29-1.9l-1.63-1.61a2.761 2.761 0 0 0 .3-.68h2.28a4.871 4.871 0 0 1-.95 2.29zm-1.33-3.66a2.6 2.6 0 0 0-.29-.69l1.62-1.6a4.829 4.829 0 0 1 .95 2.29h-2.28zm54.58-7.2a7.9 7.9 0 0 0-7.94 7.87 6.411 6.411 0 0 0 .07.99 7.938 7.938 0 0 0 15.77-.32c.02-.22.04-.44.04-.67a7.906 7.906 0 0 0-7.94-7.87zm-3.98 4.91l1.62 1.6a2.6 2.6 0 0 0-.29.69h-2.28a4.829 4.829 0 0 1 .95-2.29zm-.96 3.66h2.3a2.406 2.406 0 0 0 .29.68l-1.63 1.61a4.927 4.927 0 0 1-.96-2.29zm4.25 4.19a4.973 4.973 0 0 1-2.31-.94l1.62-1.6a2.61 2.61 0 0 0 .69.27v2.27zm0-7.51a3.122 3.122 0 0 0-.7.29l-1.62-1.6a4.938 4.938 0 0 1 2.32-.95v2.26zm1.37-2.26a4.938 4.938 0 0 1 2.32.95l-1.61 1.6a3.413 3.413 0 0 0-.71-.29v-2.26zm0 9.77v-2.27a2.328 2.328 0 0 0 .69-.28l1.62 1.61a5.036 5.036 0 0 1-2.31.94zm3.29-1.9l-1.63-1.61a2.761 2.761 0 0 0 .3-.68h2.28a4.871 4.871 0 0 1-.95 2.29zm-1.33-3.66a2.6 2.6 0 0 0-.29-.69l1.62-1.6a4.731 4.731 0 0 1 .95 2.29H1450zm20.7-4.97h-1.56l-.41-9.33a1.057 1.057 0 0 0-1.08-1.02l-21.64.19v-7.75a1.306 1.306 0 0 0-1.31-1.3h-1.04a1.306 1.306 0 0 0-1.31 1.3v7.79l-2.87.02-.58-6.86a2.787 2.787 0 0 0-2.78-2.54h-26.28a3.6 3.6 0 0 0-2.46.98l-8.82 8.4-16.89 1.77a8.738 8.738 0 0 0-5.94 3.24l-1.74 2.19a3.628 3.628 0 0 0-1.65 3.87l.53 2.35a2.653 2.653 0 0 0 2.05 2.02l5.76 1.25c-.03-.32-.06-.64-.06-.93a9.515 9.515 0 0 1 19.03 0c0 .27-.02.54-.04.8l-.03.2h38.33a8.862 8.862 0 0 1-.07-1 9.515 9.515 0 0 1 19.03 0c0 .27-.02.54-.04.8 0 .07-.02.14-.02.2h4.82l4.38-2.39h4.69a1.287 1.287 0 0 0 1.3-1.28v-1.69a1.287 1.287 0 0 0-1.3-1.28zm-65.22-9.61a4.287 4.287 0 0 0 0-2.88l3.48-4.28a1.06 1.06 0 0 1 .81-.38h10.43v7.54h-14.72zm18.57 0v-7.54h9.46a1.052 1.052 0 0 1 1.05.99l.27 6.55h-10.78z"
                                      transform="translate(-1372.25 -5538)" />
                            </svg>
                        </div>
                        <span>125</span>
                        <span>Pickup</span>
                    </a>
                </li>
                <li class="car-list__item">
                    <a href="">
                        <div class="car-list__item-image">
                            <svg xmlns="http://www.w3.org/2000/svg" width="98.88" height="35.5" viewBox="0 0 98.88 35.5">
                                <defs>
                                    <style>
                                        .cls-1 {
                                            fill: #8d99ae;
                                            fill-rule: evenodd
                                        }
                                    </style>
                                </defs>
                                <path id="Forma_1" data-name="Forma 1" class="cls-1" d="M1619.38 5556.93a7.53 7.53 0 1 0 7.43 8.17c.02-.21.03-.42.03-.64a7.494 7.494 0 0 0-7.46-7.53zm-3.74 4.69l1.52 1.54a2.77 2.77 0 0 0-.27.66h-2.14a4.711 4.711 0 0 1 .89-2.2zm-.9 3.5h2.16a2.546 2.546 0 0 0 .27.66l-1.52 1.53a4.761 4.761 0 0 1-.91-2.19zm3.99 4.01a4.62 4.62 0 0 1-2.16-.9l1.52-1.53a2.532 2.532 0 0 0 .64.26v2.17zm0-7.17a2.48 2.48 0 0 0-.65.27l-1.52-1.53a4.528 4.528 0 0 1 2.17-.91v2.17zm1.3-2.17a4.605 4.605 0 0 1 2.18.91l-1.52 1.53a2.7 2.7 0 0 0-.66-.27v-2.17zm0 9.34v-2.17a2.391 2.391 0 0 0 .64-.26l1.52 1.53a4.62 4.62 0 0 1-2.16.9zm3.09-1.81l-1.53-1.54a2.546 2.546 0 0 0 .27-.66h2.15a4.818 4.818 0 0 1-.89 2.2zm-1.25-3.5a2.77 2.77 0 0 0-.27-.66l1.52-1.54a4.711 4.711 0 0 1 .89 2.2h-2.14zm59.47-6.89a7.53 7.53 0 1 0 7.43 8.17c.01-.21.03-.42.03-.64a7.494 7.494 0 0 0-7.46-7.53zm-3.74 4.69l1.52 1.54a2.411 2.411 0 0 0-.27.66h-2.14a4.711 4.711 0 0 1 .89-2.2zm-.9 3.5h2.16a2.546 2.546 0 0 0 .27.66l-1.53 1.53a4.645 4.645 0 0 1-.9-2.19zm3.99 4.01a4.578 4.578 0 0 1-2.16-.9l1.51-1.53a2.456 2.456 0 0 0 .65.26v2.17zm0-7.17a2.411 2.411 0 0 0-.66.27l-1.52-1.53a4.564 4.564 0 0 1 2.18-.91v2.17zm1.29-2.17a4.523 4.523 0 0 1 2.18.91l-1.52 1.53a2.79 2.79 0 0 0-.66-.28v-2.16zm0 9.34v-2.17a2.456 2.456 0 0 0 .65-.26l1.52 1.53a4.657 4.657 0 0 1-2.17.9zm3.09-1.81l-1.53-1.54a2.572 2.572 0 0 0 .28-.66h2.14a4.609 4.609 0 0 1-.89 2.2zm-1.25-3.5a2.411 2.411 0 0 0-.27-.66l1.52-1.54a4.622 4.622 0 0 1 .9 2.2h-2.15zm15.43-10.79v-5.57a1.807 1.807 0 0 0-1.52-1.79 72.613 72.613 0 0 1-7.52-1.48c-2.4-.75-11.69-4.72-14.9-5.46s-17.73-4.73-31.62.13c-2.35.82-12.92 5.93-19.14 8.58-1.83.04-19.96 3.85-20.93 6.46a41.82 41.82 0 0 0-1.44 4.55 10.825 10.825 0 0 0 1.34 6.05c1.38.78 4.02 1.31 7.07 1.66-.03-.19-.07-.37-.09-.56a7.726 7.726 0 0 1-.08-1.14 8.96 8.96 0 1 1 17.92 0c0 .26-.02.51-.04.76a8.067 8.067 0 0 1-.29 1.63h.03l44.47-.91-.06-.34a9.281 9.281 0 0 1-.07-1.14 8.96 8.96 0 1 1 17.92 0 .433.433 0 0 1-.01.05l.19-.01 7.61-1.45a6.527 6.527 0 0 0 2.91-5.86 5.2 5.2 0 0 0-1.75-4.16zm-44.12-5.28l-16.61.52c.31-2.52-1.17-3.08-1.17-3.08 6.6-6.05 19.91-6.25 19.91-6.25zm18.64-.76l-15.13.51 1.37-8.56c7.82-.1 12.29 1.64 12.29 1.64l2.23 3.64zm8.54-.06h-4.22l-4.22-5.84c3.17 1.27 5.7 2.52 7.21 3.31a2.4 2.4 0 0 1 1.23 2.53z"
                                      transform="translate(-1602.12 -5536.5)" />
                            </svg>
                        </div>
                        <span>125</span>
                        <span>Sedans</span>
                    </a>
                </li>
            </ul>
        </div>
    </section>
    <section class="main-outtro">
        <div class="container">
            <div class="auto-news">
                <header>
                    <h2>Auto
                        <span>News</span>
                    </h2>
                </header>
                <ul class="auto-news__list">
                    <li class="news-item">
                        <a href="">
                            <div class="news-item__left">
                                <div class="news-item__image">
                                    <img src="" alt="">
                                </div>
                                <p class="news-item__date">29 Jan 2016</p>
                            </div>
                            <div class="news-item__info">
                                <p class="news-item__author">by
                                    <span>Sharon Alvarado</span>
                                </p>
                                <p class="news-item__title">Proin ut tellus sit amet nisi consectetur luctus</p>
                                <p class="news-item__excerpt">In hac habitasse platea dictumst. Nulla interdum erat quam, vel lobortis urna euismod eu. Praesent porttitor
                                    ex sed cursus rhoncus.
                                </p>
                            </div>
                        </a>
                    </li>
                    <li class="news-item">
                        <a href="">
                            <div class="news-item__left">
                                <div class="news-item__image">
                                    <img src="" alt="">
                                </div>
                                <p class="news-item__date">29 Jan 2016</p>
                            </div>
                            <div class="news-item__info">
                                <p class="news-item__author">by
                                    <span>Sharon Alvarado</span>
                                </p>
                                <p class="news-item__title">Proin ut tellus sit amet nisi consectetur luctus</p>
                                <p class="news-item__excerpt">In hac habitasse platea dictumst. Nulla interdum erat quam, vel lobortis urna euismod eu. Praesent porttitor
                                    ex sed cursus rhoncus.
                                </p>
                            </div>
                        </a>
                    </li>
                    <li class="news-item">
                        <a href="">
                            <div class="news-item__left">
                                <div class="news-item__image">
                                    <img src="" alt="">
                                </div>
                                <p class="news-item__date">29 Jan 2016</p>
                            </div>
                            <div class="news-item__info">
                                <p class="news-item__author">by
                                    <span>Sharon Alvarado</span>
                                </p>
                                <p class="news-item__title">Proin ut tellus sit amet nisi consectetur luctus</p>
                                <p class="news-item__excerpt">In hac habitasse platea dictumst. Nulla interdum erat quam, vel lobortis urna euismod eu. Praesent porttitor
                                    ex sed cursus rhoncus.
                                </p>
                            </div>
                        </a>
                    </li>
                </ul>
                <a href="" class="auto-new__all-link">All News</a>
            </div>
            <div class="special-car">
                <header>
                    <h2>Special
                        <span>Car</span>
                    </h2>
                </header>
                <div class="special-car__info">
                    <div class="special-car__top">
                        <div class="special-car__image">
                            <img src="" alt="">
                        </div>
                        <div class="special-car__price">
                            <p>$45</p>
                            <p>
                                <span>Per Day</span>
                                <span>To rent for a week</span>
                            </p>
                        </div>
                    </div>
                    <div class="special-car__bottom">
                        <ul class="special-car__features">
                            <li>
                                <span>Class:</span>
                                <span>Premium</span>
                            </li>
                            <li>
                                <span>Body Style:</span>
                                <span>Sedan</span>
                            </li>
                            <li>
                                <span>Transmission:</span>
                                <span>Automatic</span>
                            </li>
                            <li>
                                <span>Air Condition:</span>
                                <span>Yes</span>
                            </li>
                        </ul>
                        <a href="" class="special-car__cta">Reserve</a>
                    </div>
                </div>
            </div>
        </div>
    </section>
</main>

<script type="text/javascript" src="//code.jquery.com/jquery-1.11.0.min.js"></script>
<script type="text/javascript" src="//code.jquery.com/jquery-migrate-1.2.1.min.js"></script>
<script type="text/javascript" src="//cdn.jsdelivr.net/npm/slick-carousel@1.8.1/slick/slick.min.js"></script>
<script type="text/javascript" src="<?php echo base_url(); ?>assets/js/main.js"></script>
</body>

</html>